...
export const downloadLinkKind = pgEnum("DownloadLinkKind", [
  "EXCEL_LIFE_STYLE_COLLECTION",
  "EXCEL_COUPON",
  "EXCEL_RESERVATION",
]);
export const downloadLinkStatus = pgEnum("DownloadLinkStatus", [
  "PENDING",
  "OK",
  "FAILED",
]);
export const lifeStyleCollectionItemType = pgEnum(
  "LifeStyleCollectionItemType",
  ["SELECT", "MULTI_SELECT", "DATE", "INPUT", "TEXTAREA"]
);
export const lifeStyleCollectionPageKind = pgEnum(
  "LifeStyleCollectionPageKind",
  ["STEP", "LIST"]
);
export const personalTraitsKind = pgEnum("PersonalTraitsKind", [
  "LIFE_STYLE_COLLECTION",
]);
export const reservationHostKind = pgEnum("ReservationHostKind", [
  "ART_CENTER",
]);
export const reservationStatus = pgEnum("ReservationStatus", [
  "PENDING",
  "CONFIRMED",
  "USED",
  "CANCELLED",
...
export const lifeStyleCollectionItemPage = pgTable(
  ...
  (table) => [
    uniqueIndex("lifeStyleCollectionItemPage_itemId_pageId_key").using(
      "btree",
      table.itemId.asc().nullsLast().op("uuid_ops"),
      table.pageId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("lifeStyleCollectionItemPage_pageId_index_key").using(
      "btree",
      table.pageId.asc().nullsLast().op("text_ops"),
      table.index.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
...
export const lifeStyleCollectionPage = pgTable(
  ...
  (table) => [
    uniqueIndex("lifeStyleCollectionPage_index_key").using(
      "btree",
      table.index.asc().nullsLast().op("text_ops")
    ),
  ]
...
export const personalTraits = pgTable(
  "personalTraits",
  {
    id: uuid().defaultRandom().notNull(),
  ...
  (table) => [
    uniqueIndex("personalTraits_kind_memberId_key").using(
      "btree",
      table.kind.asc().nullsLast().op("uuid_ops"),
      table.memberId.asc().nullsLast().op("uuid_ops")
    ),
    index("personalTraits_values_idx").using(
      "gin",
      table.values.asc().nullsLast().op("jsonb_ops")
    ),
    foreignKey({
...
export const adminTrail = pgTable(
  "adminTrail",
  {
    id: uuid().defaultRandom().notNull(),
  ...
  (table) => [
    index("adminTrail_value_idx").using(
      "gin",
...
export const reservationSpace = pgTable(
  "reservationSpace",
  {
    id: uuid().defaultRandom().notNull(),
    name: varchar({ length: 35 }).notNull(),
...
export const activity = pgTable(
  ...
  (table) => [
    index("activity_value_idx").using(
      "gin",
      table.value.asc().nullsLast().op("jsonb_ops")
    ),
  ]
...
export const reservationContent = pgTable(
  "reservationContent",
  {
    id: uuid().defaultRandom().notNull(),
    name: varchar({ length: 35 }).notNull(),
  ...
  (table) => [
    index("reservationContent_isActive_displayStartAt_displayEndAt_idx").using(
      "btree",
      table.isActive.asc().nullsLast().op("bool_ops"),
      table.displayStartAt.asc().nullsLast().op("bool_ops"),
      table.displayEndAt.asc().nullsLast().op("timestamp_ops")
    ),
    foreignKey({
      columns: [table.hostId],
      foreignColumns: [reservationHost.id],
...
export const reservationTimeSlot = pgTable(
  ...
  (table) => [
    index("reservationTimeSlot_contentId_isActive_startAt_idx").using(
      "btree",
      table.contentId.asc().nullsLast().op("uuid_ops"),
      table.isActive.asc().nullsLast().op("uuid_ops"),
      table.startAt.asc().nullsLast().op("bool_ops")
    ),
    foreignKey({
...
export const reservation = pgTable(
  ...
  (table) => [
    uniqueIndex("reservation_couponPoolId_key").using(
      "btree",
      table.couponPoolId.asc().nullsLast().op("uuid_ops")
    ),
    index("reservation_memberId_status_idx").using(
      "btree",
      table.memberId.asc().nullsLast().op("uuid_ops"),
      table.status.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("reservation_sn_key").using(
      "btree",
      table.sn.asc().nullsLast().op("text_ops")
    ),
    index("reservation_timeSlotId_status_idx").using(
      "btree",
      table.timeSlotId.asc().nullsLast().op("uuid_ops"),
      table.status.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
...
export const terms = pgTable(
  ...
  (table) => [
    uniqueIndex("terms_kind_index_key").using(
      "btree",
      table.kind.asc().nullsLast().op("text_ops"),
      table.index.asc().nullsLast().op("enum_ops")
    ),
    uniqueIndex("terms_sn_rev_key").using(
      "btree",
      table.sn.asc().nullsLast().op("text_ops"),
      table.rev.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
...
export const couponPool = pgTable(
  ...
  (table) => [
    index("couponPool_memberId_status_idx").using(
      "btree",
      table.memberId.asc().nullsLast().op("uuid_ops"),
      table.status.asc().nullsLast().op("uuid_ops")
    ),
...
export const reservationHist = pgTable(
  "reservationHist",
  {
    id: serial().notNull(),
    status: reservationStatus().notNull(),
  ...
  (table) => [
    index("reservationHist_reservationId_createdAt_idx").using(
      "btree",
      table.reservationId.asc().nullsLast().op("timestamp_ops"),
