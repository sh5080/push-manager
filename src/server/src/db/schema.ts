...
export const pushSendLog = pgTable("pushSendLog", {
  ...
  oneSignalPushId: varchar({ length: 255 }),
  totalCount: integer().notNull(),
  currentCount: integer().notNull().default(0),
  lastExternalId: varchar({ length: 255 }),
...
export const pushSendErrorLog = pgTable(
  "pushSendErrorLog",
  {
    id: serial().primaryKey(),
    pushSendLogId: integer().notNull(),
    message: text(),
    createdAt: timestamp({ precision: 3, mode: "string" })
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
  ...
  (table) => [
    foreignKey({
      columns: [table.pushSendLogId],
      foreignColumns: [pushSendLog.id],
      name: "pushSendErrorLog_pushSendLogId_fkey",
    }).onDelete("restrict"),
  ]
);
