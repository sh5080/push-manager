...
export class AppSettingService implements IAppSettingService {
  ...
  async getActivity(dto: GetActivityDto) {
    try {
      ...
      // 4. ci -> bestshopNm 매핑 정보 조회 (Promise.all 사용)
      ...
      // 5. 최종 결과 매핑
      const enhancedData: IActivityWithBestshopNm[] = activities.data.map(
        (item) => {
          const memNo = item.value.memNo;
          const bestshopNm = memNo
            ? memNoToBestshopNmMap.get(memNo)
            : undefined;
